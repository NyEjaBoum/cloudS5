...
1. Lancer le serveur de développement Vite (web):

```bash
npm run dev
```

1. Pour tester sur appareil/émulateur natif pendant le développement, utiliser Capacitor (optionnelle live reload via `ionic cap run` ou méthodes similaires):
...
1. Synchroniser le dossier `dist` avec le projet Android:

```bash
npx cap sync android
```

1. Ouvrir le projet Android et construire depuis Android Studio (recommandé):
...
1. Le binaire (APK/AAB) généré se trouve dans `mobile/app/android/app/build/outputs/`.
